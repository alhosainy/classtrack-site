...
const ICONS = {
  ...
  tests:
    '<path d="M3 3v18h18"/><path d="m7 15 3.2-3.8 2.4 2.3L18 7"/>',
  reports:
    '<path d="M21 12a8 8 0 0 1-8 8H4l1.5-2.5A8 8 0 1 1 21 12Z"/><path d="M8 10.5h8M8 14h5"/>',
  qr:
    '<rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><path d="M14 14h3v3h-3zM20 14h1M17 20v1M21 18v3"/>',
  offline:
  ...
  shield:
    '<path d="M12 22s8-3.5 8-9V5l-8-3-8 3v8c0 5.5 8 9 8 9Z"/>',
  bell:
    '<path d="M18 9a6 6 0 0 0-12 0c0 7-3 8-3 8h18s-3-1-3-8"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/>',
...
const I18N = {
  ar: {
    meta: { title: 'ClassTrack — كل ما يحتاجه المدرّس لإدارة حصصه', desc: 'أدِر مجموعاتك، الحضور، الدفعات، والاختبارات في تطبيق واحد يعمل بدون إنترنت' },
    ...
    features: {
      ...
      items: [
        { icon: 'rollcall', title: 'سجل الحضور', desc: 'حاضر / غائب / متأخر مع تقييم الواجب بلمسة واحدة، وزر "الكل حاضر" لمسح المجموعة في ثانية.' },
        { icon: 'groups', title: 'المجموعات والجداول', desc: 'مجموعات بجدول أسبوعي مرن مع منع تعارض الحصص، من المركز أو أونلاين.' },
        { icon: 'payments', title: 'الدفعات', desc: 'دفع شهري أو بالحصة، مع نسبة تحصيل واضحة ومتابعة لكل طالب ومجموعة.' },
        { icon: 'tests', title: 'الاختبارات', desc: 'حدّد الاختبارات، سجّل الدرجات، وشاهد المتوسطات في رسوم بيانية واضحة.' },
        { icon: 'reports', title: 'تقارير واتساب', desc: 'تقرير يومي لكل طالب وملخص درجات للمجموعة، بقالب عربي جاهز للتخصيص.' },
        { icon: 'qr', title: 'رموز QR', desc: 'لكل طالب رمز QR ثابت — امسح الرمز وافتح الملف مباشرة، وشارك الملصق على واتساب.' },
        { icon: 'cloud', title: 'نسخ احتياطي ومزامنة', desc: 'تصدير واستعادة كامل للبيانات، مع مزامنة سحابية اختيارية عند تسجيل الدخول.' },
        { icon: 'bell', title: 'تذكيرات ذكية', desc: 'تذكير قبل الحصص، تذكير بالدفعات، وتنبيه عند تكرار غياب الطالب.' },
      ],
    },
    ...
    how: {
      ...
      steps: [
        { title: 'أنشئ مجموعاتك', desc: 'أضف مجموعات بمواعيدها الأسبوعية ثم الطلاب — واحدًا تلو الآخر أو دفعة واحدة.' },
        { title: 'تابع يوميًا', desc: 'سجّل الحضور والواجبات، تابع الدفعات، وسجّل درجات الاختبارات أثناء الحصة.' },
        { title: 'شارك النتائج', desc: 'أرسل التقارير اليومية وملخص الدرجات للأهالي عبر واتساب بضغطة واحدة.' },
      ],
    },
  ...
  en: {
    meta: { title: 'ClassTrack — Everything a private teacher needs', desc: 'Manage your groups, roll-call, payments, and tests in one app that works offline' },
    ...
    features: {
      ...
      items: [
        { icon: 'rollcall', title: 'Roll-call', desc: 'Present / Absent / Late plus homework quality in one tap, with a "mark all present" shortcut to clear the room in a second.' },
        { icon: 'groups', title: 'Groups & schedules', desc: 'Groups with a flexible weekly schedule and automatic conflict detection — center or online.' },
        { icon: 'payments', title: 'Payments', desc: 'Monthly or per-session billing, with a clear collection rate per student and group.' },
        { icon: 'tests', title: 'Tests & scores', desc: 'Define tests, record scores, and watch averages in clear progress charts.' },
        { icon: 'reports', title: 'WhatsApp reports', desc: 'A daily report per student and a group grades summary, with a customizable Arabic template.' },
        { icon: 'qr', title: 'QR codes', desc: 'Every student gets a stable QR — scan to open their profile instantly and share the label on WhatsApp.' },
        { icon: 'cloud', title: 'Backup & sync', desc: 'Full export and restore, with optional cloud sync once you sign in.' },
        { icon: 'bell', title: 'Smart reminders', desc: 'Lesson reminders, payment due reminders, and alerts when a student crosses the absence threshold.' },
      ],
    },
    ...
    how: {
      ...
      steps: [
        { title: 'Set up your groups', desc: 'Add groups with their weekly schedule, then add students — one by one or in bulk.' },
        { title: 'Track it daily', desc: 'Take roll-call, follow payments, and log test scores as class runs.' },
        { title: 'Share the results', desc: 'Send daily reports and grade summaries to parents over WhatsApp in one tap.' },
      ],
    },
...
function renderFeatures() {
  document.getElementById('featuresGrid').innerHTML = t('features.items')
    .map(
      (f) => `<article class="feature-card"><span class="feature-icon">${svg(ICONS[f.icon])}</span><h3>${f.title}</h3><p>${f.desc}</p></article>`
    )
    .join('');
}
...
function renderHow() {
  document.getElementById('howGrid').innerHTML = t('how.steps')
    .map((s) => `<div class="how-step"><h3>${s.title}</h3><p>${s.desc}</p></div>`)
    .join('');
}
...
function renderGallery() {
  document.getElementById('galleryGrid').innerHTML = t('screens.items')
    .map(
      (g) => `<figure class="gallery-item"><div class="mini-phone"><div class="phone-screen"><img src="assets/screenshots/${g.img}" alt="${g.caption}" loading="lazy"></div></div><figcaption>${g.caption}</figcaption></figure>`
    )
    .join('');
}
...
function renderCarousel() {
  ...
  screen.innerHTML = slides
    .map((s, i) => `<img src="assets/screenshots/${s.img}" alt="${s.caption}" class="${i === 0 ? 'active' : ''}">`)
    .join('');

  const dots = document.getElementById('carouselDots');
  dots.innerHTML = slides
    .map((_, i) => `<button data-slide="${i}" aria-label="slide ${i + 1}" class="${i === 0 ? 'active' : ''}"></button>`)
    .join('');
  ...
  window._carouselTimer = setInterval(() => next((current + 1) % slides.length), 4500);
...
function applyTheme() {
  ...
  document.getElementById('themeToggle').innerHTML = svg(ICONS[dark ? 'sun' : 'moon']);
  if (dark) {
    document.querySelector('meta[name="theme-color"]')?.setAttribute('content', '#12161b');
  } else {
...
const APK_FALLBACK = 'https://github.com/alhosainy/classtrack-site/releases/latest';
const SITE_API = 'https://api.github.com/repos/alhosainy/classtrack-site/releases/latest';
...
async function setApkLinks() {
  ...
  try {
    ...
    const asset = (release.assets || []).find((a) => /^classtrack-.*\.apk$/.test(a.name));
    links.forEach((a) => { a.href = asset?.browser_download_url || APK_FALLBACK; });
  } catch {
    links.forEach((a) => { a.href = APK_FALLBACK; });
  }
}
